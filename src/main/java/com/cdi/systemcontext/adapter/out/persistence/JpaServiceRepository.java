 ...
 * service table, data-model.md §3.A, V8 migration).
 *
 * <p>Persistence-side only: no domain rules here. The tenant-scoped natural-key
 * lookup that drives UC-09 idempotent reuse delegates to the Spring Data
 * derived query; {@code UNIQUE (tenant_id, name)} (V8) remains the DB safety
